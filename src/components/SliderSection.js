...
function SliderSection() {
  ...
  return (
    <div className="flex flex-col w-full lg:w-[1440px] px-4 lg:px-[104px] pt-10 items-start gap-4">
      {/* Buttons */}
        ...
        {["Show All", "City", "Forest", "Water"].map((label) => (
          <button
            ...
            className={`flex items-center justify-center h-10 px-4 py-2 text-sm font-bold leading-6 rounded-full bg-[#F2F2F2] text-[#4B4B4B] border border-transparent
              ${
                label === activeButton
                  ? "bg-[#F2F2F2] text-[#4B4B4B] border-[#4B4B4B]"
                  : "hover:bg-[#A0A0A0] hover:text-[#4B4B4B]"
              }
            `}
            onClick={() =>
      ...
      {/* Carousel */}
      <div
        className="flex overflow-x-hidden relative w-full h-[393px] overflow-hidden"
        {...handlers}>
        <div
          className="flex transition-transform duration-300"
          style={{
            transform: `translateX(-${(294.5 + 18) * currentIndex}px)`,
          }}>
          {images.map((imgSrc, index) => (
            <div
              key={index}
              className="relative w-[294.5px] h-[393px] overflow-hidden rounded-lg mr-[18px] last:mr-0">
              <Image
                src={imgSrc}
              ...
              />
              <button className="absolute left-[75px] bottom-[36.333px] flex items-center justify-center h-12 px-6 bg-white rounded-full">
                Lorem Ipsum
              </button>
            </div>
          ))}
      ...
      {/* Progress Bar */}
      <div className="relative w-full lg:w-[1232px] h-[2px] mb-[40px] mx-auto bg-[#F2F2F2]">
        <div
          className="absolute left-0 top-0 h-full bg-[#4B4B4B]"
